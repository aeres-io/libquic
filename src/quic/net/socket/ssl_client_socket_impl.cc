...
int SSLClientSocketImpl::DoHandshake() {
  ...
  if (rv <= 0) {
    ...
    net_error = MapLastOpenSSLError(ssl_error, &error_info);
...
int SSLClientSocketImpl::DoPayloadWrite() {
  ...
  if (ssl_error == SSL_ERROR_WANT_PRIVATE_KEY_OPERATION)
    return ERR_IO_PENDING;
  OpenSSLErrorInfo error_info;
  int net_error = MapLastOpenSSLError(ssl_error, &error_info);
...
int SSLClientSocketImpl::ClientCertRequestCallback(SSL* ssl) {
  ...
  if (ssl_config_.client_cert.get()) {
    if (!ssl_config_.client_private_key) {
      ...
      OpenSSLPutNetError(ERR_SSL_CLIENT_AUTH_CERT_NO_PRIVATE_KEY);
      return -1;
    }

    if (!SetSSLChainAndKey(ssl_.get(), ssl_config_.client_cert.get(), nullptr,
                           &SSLContext::kPrivateKeyMethod)) {
      OpenSSLPutNetError(ERR_SSL_CLIENT_AUTH_CERT_BAD_FORMAT);
      return -1;
    }
...
ssl_private_key_result_t SSLClientSocketImpl::PrivateKeyCompleteCallback(
  ...
  if (signature_result_ != OK) {
    OpenSSLPutNetError(signature_result_);
    return ssl_private_key_failure;
  }
  if (signature_.size() > max_out) {
    OpenSSLPutNetError(ERR_SSL_CLIENT_AUTH_SIGNATURE_FAILED);
    return ssl_private_key_failure;
  }
...
int SSLClientSocketImpl::MapLastOpenSSLError(
  ...
  int net_error = MapOpenSSLErrorWithDetails(ssl_error, info);
