  ...
  if (header.nonce != nullptr) {
    os << ", diversification_nonce: "
       << QuicTextUtils::HexEncode(
              base::StringPiece(header.nonce->data(), header.nonce->size()));
  }
...
base::StringPiece QuicPacket::AssociatedData(QuicTransportVersion version) const {
...
base::StringPiece QuicPacket::Plaintext(QuicTransportVersion version) const {
  const size_t start_of_encrypted_data = GetStartOfEncryptedData(
      version, connection_id_length_, includes_version_,
      includes_diversification_nonce_, packet_number_length_);
  return base::StringPiece(data() + start_of_encrypted_data,
