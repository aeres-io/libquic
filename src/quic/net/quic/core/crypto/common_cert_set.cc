...
int Compare(base::StringPiece a, const unsigned char* b, size_t b_len) {
  size_t len = a.size();
  if (len > b_len) {
    len = b_len;
  }
...
class CommonCertSetsQUIC : public CommonCertSets {
 public:
  ...
  base::StringPiece GetCommonHashes() const override {
    return base::StringPiece(reinterpret_cast<const char*>(kSetHashes),
                           sizeof(uint64_t) * QUIC_ARRAYSIZE(kSetHashes));
  }

  base::StringPiece GetCert(uint64_t hash, uint32_t index) const override {
  ...
  bool MatchCert(base::StringPiece cert,
