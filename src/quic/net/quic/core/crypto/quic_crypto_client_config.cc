...
QuicCryptoClientConfig::CachedState::SetServerConfig(
    base::StringPiece server_config,
    QuicWallTime now,
...
void QuicCryptoClientConfig::FillInchoateClientHello(
  ...
  // per the spec.
  //LiuXiong: We need to re-write this code with openssl
  ...
  out->SetStringPiece(
      kNONP, base::StringPiece(proof_nonce, QUIC_ARRAYSIZE(proof_nonce)));
...
QuicErrorCode QuicCryptoClientConfig::FillClientHello(
  ...
  if (channel_id_key) {
    ...
    if (!crypters.encrypter->EncryptPacket(
            ...
            base::StringPiece() /* associated data */,
    ...
    out->SetStringPiece(kCETV, base::StringPiece(output.get(), output_size));
    out->MarkDirty();
...
QuicErrorCode QuicCryptoClientConfig::CacheNewServerConfig(
  ...
  base::StringPiece proof, cert_bytes, cert_sct;
