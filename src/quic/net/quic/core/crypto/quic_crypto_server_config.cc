...
string DeriveSourceAddressTokenKey(
  ...
  crypto::HKDF hkdf(
      source_address_token_secret, base::StringPiece() /* no salt */,
...
QuicCryptoServerConfig::GenerateConfig(QuicRandom* rand,
  ...
  base::StringPiece curve25519_public_value = curve25519->public_value();
  ...
  if (options.p256) {
    ...
    base::StringPiece p256_public_value = p256->public_value();
  ...
  msg.SetStringPiece(kORBT, base::StringPiece(orbit_bytes, sizeof(orbit_bytes)));
  ...
  if (options.id.empty()) {
    ...
    msg.SetStringPiece(
        kSCID, base::StringPiece(reinterpret_cast<const char*>(scid_bytes), 16));
  } else {
...
void QuicCryptoServerConfig::ValidateClientHello(
  ...
  base::StringPiece requested_scid;
...
void QuicCryptoServerConfig::ProcessClientHelloAfterGetProof(
  ...
  base::StringPiece cetv_ciphertext;
  if (requested_config->channel_id_enabled &&
    ...
    const bool success = crypters.decrypter->DecryptPacket(
        ...
        base::StringPiece() /* associated data */, cetv_ciphertext, plaintext,
    ...
    std::unique_ptr<CryptoHandshakeMessage> cetv(CryptoFramer::ParseMessage(
        base::StringPiece(plaintext, plaintext_length), Perspective::IS_SERVER));
    if (!cetv.get()) {
      helper.Fail(QUIC_INVALID_CRYPTO_MESSAGE_PARAMETER, "CETV parse error");
      return;
    }

    base::StringPiece key, signature;
...
void QuicCryptoServerConfig::EvaluateClientHello(
  ...
  if (need_proof) {
    ...
    std::unique_ptr<EvaluateClientHelloCallback> cb(
        new EvaluateClientHelloCallback(
            *this, found_error, server_address.address(), version,
  ...
  EvaluateClientHelloAfterGetProof(
      found_error, server_address.address(), version, requested_config,
...
string QuicCryptoServerConfig::NewSourceAddressToken(
  ...
  source_address_token->set_ip(IPAddressToPackedString(ip.DualStacked()));
...
HandshakeFailureReason QuicCryptoServerConfig::ValidateSingleSourceAddressToken(
  ...
  if (source_address_token.ip() != IPAddressToPackedString(ip.DualStacked())) {
...
string QuicCryptoServerConfig::NewServerNonce(QuicRandom* rand,
  ...
  return server_nonce_boxer_.Box(
      rand, base::StringPiece(reinterpret_cast<char*>(server_nonce),
                            sizeof(server_nonce)));
}
