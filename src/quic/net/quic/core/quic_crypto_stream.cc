...
void QuicCryptoStream::OnDataAvailable() {
  ...
  while (true) {
    ...
    base::StringPiece data(static_cast<char*>(iov.iov_base), iov.iov_len);
...
bool QuicCryptoStream::ExportKeyingMaterial(base::StringPiece label,
...
void QuicCryptoStream::WriteCryptoData(const base::StringPiece& data) {
