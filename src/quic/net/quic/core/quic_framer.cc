...
std::unique_ptr<QuicEncryptedPacket> QuicFramer::BuildPublicResetPacket(
  ...
  if (packet.client_address.GetFamily() !=
...
bool QuicFramer::ProcessStreamFrame(QuicDataReader* reader,
  ...
  // TODO(ianswett): Don't use base::StringPiece as an intermediary.
...
bool QuicFramer::ProcessGoAwayFrame(QuicDataReader* reader,
  ...
  base::StringPiece reason_phrase;
...
// static
base::StringPiece QuicFramer::GetAssociatedDataFromEncryptedPacket(
    QuicTransportVersion version,
...
size_t QuicFramer::EncryptInPlace(EncryptionLevel level,
  ...
  if (!encrypter_[level]->EncryptPacket(
          ...
          base::StringPiece(buffer, ad_len),  // Associated data
          base::StringPiece(buffer + ad_len, total_len - ad_len),  // Plaintext
...
bool QuicFramer::DecryptPayload(QuicDataReader* encrypted_reader,
  ...
  base::StringPiece encrypted = encrypted_reader->ReadRemainingPayload();
  base::StringPiece associated_data = GetAssociatedDataFromEncryptedPacket(
...
base::StringPiece QuicFramer::TruncateErrorString(base::StringPiece error) {
  if (error.length() <= kMaxErrorStringLength) {
    return error;
  }
  return base::StringPiece(error.data(), kMaxErrorStringLength);
}
