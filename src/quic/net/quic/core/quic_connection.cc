...
void QuicConnection::SendVersionNegotiationPacket() {
  ...
  WriteResult result = writer_->WritePacket(
      version_packet->data(), version_packet->length(), self_address().address(),
...
void QuicConnection::ProcessUdpPacket(const IPEndPoint& self_address,
  ...
  if (self_address_.GetFamily() == (AddressFamily)net::ADDRESS_FAMILY_UNSPECIFIED) {
    self_address_ = last_packet_destination_address_;
  }
  if (peer_address_.GetFamily() == (AddressFamily)net::ADDRESS_FAMILY_UNSPECIFIED) {
    peer_address_ = last_packet_source_address_;
  }
...
bool QuicConnection::ProcessValidatedPacket(const QuicPacketHeader& header) {
  if (perspective_ == Perspective::IS_SERVER && self_address_.GetFamily() != net::ADDRESS_FAMILY_UNSPECIFIED &&
      last_packet_destination_address_.GetFamily() != net::ADDRESS_FAMILY_UNSPECIFIED &&
      !(self_address_ == last_packet_destination_address_)) {
    ...
    if (self_address_.port() != last_packet_destination_address_.port() ||
        self_address_.address().Normalized() !=
            last_packet_destination_address_.address().Normalized()) {
...
QuicByteCount QuicConnection::GetLimitedMaxPacketSize(
  ...
  if (peer_address_.GetFamily() == net::ADDRESS_FAMILY_UNSPECIFIED) {
...
bool QuicConnection::SendConnectivityProbingPacket(
  ...
  WriteResult result = probing_writer->WritePacket(
      probing_packet->data(), probing_packet->length(), self_address().address(),
      peer_address, per_packet_options_);
...
base::StringPiece QuicConnection::GetCurrentPacket() {
  if (current_packet_data_ == nullptr) {
    return base::StringPiece();
  }
  return base::StringPiece(current_packet_data_, last_size_);
}
...
QuicBufferAllocator*
QuicConnectionHelper::GetStreamSendBufferAllocator() {
