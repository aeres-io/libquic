...
void SetKeyAndIV(const EVP_MD* prf,
  ...
  crypter->SetKey(
      base::StringPiece(reinterpret_cast<char*>(key.data()), key.size()));
  crypter->SetIV(
      base::StringPiece(reinterpret_cast<char*>(iv.data()), iv.size()));
}
...
void TlsHandshaker::OnDataReceivedFromBIO(const base::StringPiece& data) {
