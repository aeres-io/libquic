...
QuicHeadersStream::QuicHeadersStream(QuicSession* session)
    : QuicStream(kHeadersStreamId, session), session_(session) {
...
void QuicHeadersStream::OnDataAvailable() {
  ...
  while (true) {
    ...
    // TODO: (jason) Remove from QuicSpdySession

#if 0
    if (session_->ProcessHeaderData(iov, timestamp) != iov.iov_len) {
...
void QuicHeadersStream::MaybeReleaseSequencerBuffer() {
  ...
  //
#if 0
  if (session_->ShouldReleaseHeadersStreamSequencerBuffer()) {
