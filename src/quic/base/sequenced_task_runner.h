...
class BASE_EXPORT SequencedTaskRunner : public TaskRunner {
 public:
  ...
  bool PostNonNestableTask(OnceClosure task);

  virtual bool PostNonNestableDelayedTask(OnceClosure task,
  ...
  bool DeleteSoon(const T* object) {
    return DeleteOrReleaseSoonInternal(&DeleteHelper<T>::DoDelete,
                                       object);
  }
  ...
  template <class T>
  bool ReleaseSoon(const T* object) {
    return DeleteOrReleaseSoonInternal(&ReleaseHelper<T>::DoRelease,
                                       object);
  }
