...
bool SequencedTaskRunner::PostNonNestableTask(OnceClosure task) {
  return PostNonNestableDelayedTask(std::move(task),
                                    base::TimeDelta());
}
...
bool SequencedTaskRunner::DeleteOrReleaseSoonInternal(
    ...
    const void* object) {
  return PostNonNestableTask(BindOnce(deleter, object));
}
