...
std::string SysWideToNativeMB(const std::wstring& wide) {
  char * buffer = new char[wide.size() * 2 + 1];
  wcstombs(buffer, wide.c_str(), wide.size() * 2 + 1);
...
std::wstring SysNativeMBToWide(const StringPiece& native_mb) {
  wchar_t * buffer = new wchar_t[native_mb.size() + 1];
  mbstowcs(buffer, native_mb.data(), native_mb.size() + 1);
...
std::string SysWideToUTF8(const std::wstring& wide) {
  return SysWideToNativeMB(wide);
}

std::wstring SysUTF8ToWide(const StringPiece& utf8) {
  return SysNativeMBToWide(utf8);
}

#elif defined(SYSTEM_NATIVE_UTF8) || defined(OS_ANDROID)
